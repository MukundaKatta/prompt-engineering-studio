// prompt-engineering-studio — tester module
...
export class Tester extends EventEmitter {
  private readonly options: Required<TesterOptions>;
  private initialized = false;

  constructor(options: TesterOptions = {}) {
    super();
    this.options = { name: options.name ?? "Tester", enabled: options.enabled ?? true, timeout: options.timeout ?? 30000 };
  }
  ...
  async process(input: unknown): Promise<TesterResult> {
