// prompt-engineering-studio — evaluator module
...
export class Evaluator extends EventEmitter {
  ...
  constructor(options: EvaluatorOptions = {}) {
    super();
    this.options = { name: options.name ?? "Evaluator", enabled: options.enabled ?? true, timeout: options.timeout ?? 30000 };
  }
  ...
  async process(input: unknown): Promise<EvaluatorResult> {
